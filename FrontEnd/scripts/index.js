...
const createFilterButtons = (categories, allProjects) => {
  ...
  categories.forEach(cat => {
    if (cat.name === 'Tous') {
      ...
      createBtn(cat.name, () => cat.id !== 0 ? allProjects.filter(p => p.categoryId === cat.id) : null)
    }
  })
}
...
const logout = () => {
  ...
  allCatagories.forEach(cat => {
    if (cat.name === 'Tous') {
      ...
      createBtn(cat.name, () => cat.id !== 0 ? allWorks.filter(p => p.categoryId === cat.id) : null)
    }
  })
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  console.log("🚀 ~ categoerieTous - celle que je veux insérer :", categoerieTous)
